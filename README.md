...
Se você quer que o site seja acessível em "<seu-dominio>", siga estes passos básicos:

- Adicione o arquivo `CNAME` na raiz do repositório contendo o domínio desejado.
...
- O arquivo `CNAME` na raiz do repositório instrui o GitHub Pages a responder por esse domínio. Já o DNS é responsabilidade do provedor onde você registrou o domínio.
- Se preferir, eu posso:
  ...
  - Ajudar a configurar o redirecionamento do domínio raiz para `www` conforme sua preferência.

**Publicar no GitHub (passo a passo)**

Siga estes passos para publicar o site no GitHub Pages a partir da `main`:

1. Crie um repositório no GitHub (por exemplo `SEU_REPO`).
...
5. Se você deseja um domínio customizado (por exemplo `<seu-dominio>`):

  - Garanta que o arquivo `CNAME` contenha o domínio desejado na raiz do repositório.
...
6. Aguarde: após o workflow e a configuração DNS, o site deverá ficar disponível em `https://<seu-usuario>.github.io/<seu-repo>/` ou em `https://<seu-dominio>` após a propagação DNS.
